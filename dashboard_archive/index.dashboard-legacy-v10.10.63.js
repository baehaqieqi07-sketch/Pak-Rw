...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    /* ================= PAK RW SOFT CONTROL ROOM v10.10.63 ================= */
    body.dashboard-clean {
      background:
    ...
    /* ================= COMFORT PRO DASHBOARD v10.10.63 ================= */
    body.dashboard-clean {
      background:
...
/* ================= PAK RW PREMIUM DASHBOARD ONLY v10.10.63 ================= */
:root{
  --premium-bg:#070a14;
  --premium-bg2:#0b1021;
...
/* ================= PAK RW SPACIOUS PREMIUM DASHBOARD ONLY v10.10.63 ================= */
:root{
  --sp-bg:#070912;
  --sp-bg2:#0b1020;
...
/* ===== v10.10.63 Control Center Premium Safe Polish ===== */
    ...
    <div class="premium-app">
      <aside class="premium-sidebar" id="premiumSidebar">
        <div class="premium-brand">
          ...
          <div>
            <h1>${escapeHtml(dash.brandTitle || "Pak RW v10.10.63")}</h1>
      ...
      <main class="premium-main">
        <div class="premium-topbar">
          ...
          <div class="premium-toolbar" aria-label="Dashboard tools">
            ...
            <span class="premium-chip">v10.10.63</span>
          </div>
        </div>
        ${inner}
      </main>
...
function renderFeatureFlowDashboard(req) {
  ...
  return dashboardLayout(`
    <section class="suite-hero">
      <span class="ot-badge">🧭 FULL FEATURE SUITE • v10.10.63</span>
    ...
    <form method="post" action="/maxton-control" class="panel mega-dashboard-v2">
      ...
      <section class="mega-section" id="texts">
        ...
        <div class="formgrid">
          ...
          ${configInput("suggestionTitle", "Judul Panel Saran", cfg.suggestion.title || "💡 <a:Desa_Tulus2:1518502350363430932> DESA TULUS | Kritik & Saran")}
...
function applyMaxtonControlPost(body = {}) {
  ...
  cfg.suggestion.title = body.suggestionTitle || cfg.suggestion.title || "💡 <a:Desa_Tulus2:1518502350363430932> DESA TULUS | Kritik & Saran";
...
function getDashboardGuild() {
  ...
  if (preferredId) {
    const preferred = client.guilds.cache.get(preferredId);
...
const PAKRW_CHANNEL_PLACEHOLDER_BINDINGS = [
  { field: "bindRulesChannel", path: "rulesChannelId", placeholder: "{rulesChannel}", label: "Channel Aturan Desa" },
  { field: "bindChatWargaChannel", path: "chatWargaChannelId", placeholder: "{chatWargaChannel}", label: "Channel Chat Warga" },
  { field: "bindTicketChannel", path: "ticketChannelId", placeholder: "{ticketChannel}", label: "Channel Ticket" },
  { field: "bindAiChannel", path: "aiChannelId", placeholder: "{aiChannel}", label: "Channel AI Pak RW" },
  { field: "bindCurhatChannel", path: "curhatChannelId", placeholder: "{curhatChannel}", label: "Channel Curhat" },
  { field: "bindAnonymousCurhatChannel", path: "anonymousCurhatChannelId", placeholder: "{anonymousCurhatChannel}", label: "Channel Curhat Anonim" },
  { field: "bindSuggestionChannel", path: "suggestionChannelId", placeholder: "{suggestionChannel}", label: "Channel Kotak Saran" },
  { field: "bindLevelChannel", path: "levelChannelId", placeholder: "{levelChannel}", label: "Channel Level" },
  { field: "bindCekPoinChannel", path: "cekPoinChannelId", placeholder: "{cekPoinChannel}", label: "Channel Cek Poin" },
  { field: "bindTopActiveChannel", path: "topActive.channelId", placeholder: "{topActiveChannel}", label: "Channel Top Aktif Bulanan" },
  { field: "bindLeaderboardChannel", path: "papanAktif.channelId", placeholder: "{leaderboardChannel}", label: "Channel Papan Aktif Lifetime" },
  { field: "bindMabarChannel", path: "mabar.channelId", placeholder: "{mabarChannel}", label: "Channel Cari Mabar" },
  { field: "bindBoostPoinChannel", path: "boostPoin.channelId", placeholder: "{boostPoinChannel}", label: "Channel Boost Poin" },
  { field: "bindWelcomeChannel", path: "welcomeChannelId", placeholder: "{welcomeChannel}", label: "Channel Welcome" }
];

const PAKRW_ROLE_PLACEHOLDER_BINDINGS = [
  { field: "bindMemberTulusRole", path: "memberTulusRoleId", placeholder: "{memberRole} / {memberTulusRole}", label: "Role Member Tulus" },
  { field: "bindStaffRole", path: "staffRoleId", placeholder: "{staffRole}", label: "Role Staff" },
  { field: "bindAdminRole", path: "adminRoleId", placeholder: "{adminRole}", label: "Role Admin" },
  { field: "bindModeratorRole", path: "moderatorRoleId", placeholder: "{moderatorRole}", label: "Role Moderator" },
  { field: "bindMotmRole", path: "topActive.memberOfTheMonthRoleId", placeholder: "{motmRole}", label: "Role Member Of The Month" },
  { field: "bindDonaturRole", path: "donaturRoleId", placeholder: "{donaturRole}", label: "Role Donatur Desa" },
  { field: "bindJuraganRole", path: "juragan.roleId", placeholder: "{juraganRole}", label: "Role Juragan Desa" }
];
...
function pakRwChannelSelect(name, label, selected = "", guild, helper = "") {
  ...
  const options = guild ? channelOptions(guild, selectedId, "text") : "";
  const hasSelected = Boolean(selectedId && guild?.channels?.cache?.has?.(selectedId));
  const fallback = selectedId && !hasSelected
    ? `<option value="${escapeHtml(selectedId)}" selected>Channel tersimpan • ${escapeHtml(selectedId)}</option>`
    : "";
  const readable = pakRwReadableChannel(guild, selectedId);

  return `<label class="prw-field discord-live-field"><span>${escapeHtml(label)}</span>
    <select name="${escapeHtml(name)}" data-discord-live-select="channel" data-channel-scope="text" data-selected="${escapeHtml(selectedId)}">
      <option value="">⏳ Memuat channel dari Discord...</option>${fallback}${options}
    </select>
    <small><b data-discord-readable>${escapeHtml(selectedId ? `Terpilih: ${readable}` : "Belum memilih channel")}</b><br>${escapeHtml(helper || "Klik dropdown lalu pilih nama channel. Dashboard menyimpan Channel ID secara otomatis.")}</small>
  </label>`;
}

function pakRwRoleSelect(name, label, selected = "", guild, helper = "") {
  const selectedId = String(selected || "");
  const roles = guild ? [...guild.roles.cache.values()].filter((r) => r && r.name !== "@everyone" && !r.managed).sort((a, b) => b.position - a.position) : [];
  const hasSelected = Boolean(selectedId && guild?.roles?.cache?.has?.(selectedId));
  const fallback = selectedId && !hasSelected
    ? `<option value="${escapeHtml(selectedId)}" selected>Role tersimpan • ${escapeHtml(selectedId)}</option>`
    : "";
  const readable = pakRwReadableRole(guild, selectedId);

  return `<label class="prw-field discord-live-field"><span>${escapeHtml(label)}</span>
    <select name="${escapeHtml(name)}" data-discord-live-select="role" data-selected="${escapeHtml(selectedId)}">
      <option value="">⏳ Memuat role dari Discord...</option>${fallback}${roles.map((r) => `<option value="${escapeHtml(r.id)}" ${selectedId === String(r.id) ? "selected" : ""}>@${escapeHtml(r.name)}</option>`).join("")}
    </select>
    <small><b data-discord-readable>${escapeHtml(selectedId ? `Terpilih: ${readable}` : "Belum memilih role")}</b><br>${escapeHtml(helper || "Klik dropdown lalu pilih nama role. Dashboard menyimpan Role ID secara otomatis.")}</small>
  </label>`;
}
...
function pakRwDiscordPickerPayload(guild) {
  ...
  return {
    ...
    bot,
    placeholders: {
      channel: PAKRW_PLACEHOLDER_GROUPS.find(([group]) => group === "Channel")?.[1] || [],
      role: PAKRW_PLACEHOLDER_GROUPS.find(([group]) => group === "Role")?.[1] || [],
      user: PAKRW_PLACEHOLDER_GROUPS.find(([group]) => group === "User")?.[1] || [],
      server: PAKRW_PLACEHOLDER_GROUPS.find(([group]) => group === "Server")?.[1] || [],
      bot: ["{botName}", "{prefix}"]
    }
  };
}

function pakRwDiscordPickerPanel(guild) {
  const payload = JSON.stringify(pakRwDiscordPickerPayload(guild)).replace(/</g, "\\u003c");
  return `<section class="discord-data-picker">
    <div class="picker-live-head">
      <div class="panel-title"><b>Picker Discord Langsung</b><span>Pilih dari daftar channel/role asli DESA TULUS. Tidak perlu copy ID.</span></div>
      <button class="btn secondary" type="button" data-discord-refresh>↻ Muat Ulang Discord</button>
    </div>
    <div class="discord-live-status" data-discord-live-status>⏳ Menghubungkan dashboard ke Discord...</div>
    <div class="picker-steps"><span><b>1</b> Pilih Channel/Role</span><span><b>2</b> Cari namanya</span><span><b>3</b> Klik pilihan</span><span><b>4</b> Gunakan</span></div>
    <div class="picker-grid">
      <label><span>Yang mau dipilih</span><select data-picker-type><option value="channel"># Channel Discord</option><option value="role">@ Role Discord</option><option value="user">@ User / Member</option><option value="server">Server</option><option value="bot">Bot Pak RW</option></select></label>
      <label><span>Cari nama</span><input type="search" data-picker-search placeholder="Contoh: chat-warga, aturan, Member Tulus..."></label>
      <label class="picker-wide"><span>Klik nama yang benar</span><select data-picker-item size="8"></select></label>
      <label><span>Hasil yang dipakai</span><select data-picker-format><option value="mention">Tag asli Discord</option><option value="name">Nama biasa</option><option value="id">ID Discord</option></select></label>
      <label><span>Atau pilih placeholder</span><select data-picker-placeholder></select></label>
    </div>
    <div class="picker-active" data-picker-active>Untuk mengisi dropdown fitur: klik dropdown Channel/Role tujuan terlebih dahulu.</div>
    <div class="picker-result" data-picker-result>Belum ada data dipilih.</div>
    <div class="action-bar">
      <button class="btn" type="button" data-picker-use-select>✓ Gunakan untuk Dropdown Fitur</button>
      <button class="btn secondary" type="button" data-picker-insert>Masukkan Tag ke Teks</button>
      <button class="btn secondary" type="button" data-picker-placeholder-insert>Masukkan Placeholder</button>
      <button class="btn ghost" type="button" data-picker-copy>Salin</button>
    </div>
    <div class="warn-box"><b>Alur paling gampang:</b> pada form fitur, langsung klik dropdown channel lalu pilih namanya. Picker besar ini dipakai kalau daftar panjang atau ingin memasukkan tag ke Content/Description.</div>
    <div class="picker-links"><a href="/dashboard/manage/channel-manager">Buka semua pilihan channel</a><a href="/dashboard/manage/role-manager">Buka semua pilihan role</a></div>
    <script type="application/json" class="discord-picker-data">${payload}</script>
  </section>`;
}

function pakRwBindingFields(type, cfg, guild) {
  const bindings = type === "channel" ? PAKRW_CHANNEL_PLACEHOLDER_BINDINGS : PAKRW_ROLE_PLACEHOLDER_BINDINGS;
  return bindings.map((binding) => {
    const selected = pakRwGetByPath(cfg, binding.path, "");
    const helper = `${binding.placeholder} • tersimpan sebagai ID asli`;
    return type === "channel"
      ? pakRwChannelSelect(binding.field, binding.label, selected, guild, helper)
      : pakRwRoleSelect(binding.field, binding.label, selected, guild, helper);
  }).join("");
}

function pakRwBindingStatus(type, cfg, guild) {
  const bindings = type === "channel" ? PAKRW_CHANNEL_PLACEHOLDER_BINDINGS : PAKRW_ROLE_PLACEHOLDER_BINDINGS;
  return `<div class="binding-status">${bindings.map((binding) => {
    const id = pakRwGetByPath(cfg, binding.path, "");
    const readable = type === "channel" ? pakRwReadableChannel(guild, id) : pakRwReadableRole(guild, id);
    return `<div class="binding-row"><code>${escapeHtml(binding.placeholder)}</code><span>${escapeHtml(readable)}</span><b class="${id ? "valid" : "invalid"}">${id ? "Terhubung" : "Belum diatur"}</b></div>`;
  }).join("")}</div>`;
}
...
function pakRwPreviewData(cfg = readConfigFile()) {
  const guild = typeof getDashboardGuild === "function" ? getDashboardGuild() : client.guilds.cache.first();
  const channelMention = (id, fallback) => id ? `<#${id}>` : fallback;
  const roleMention = (id, fallback) => id ? `<@&${id}>` : fallback;
  const memberRoleId = cfg.welcome?.memberRoleId || cfg.memberTulusRoleId || "";
  const leaderboardId = cfg.papanAktif?.channelId || cfg.leaderboardAktif?.channelId || cfg.topActive?.leaderboardActiveChannelId || cfg.topActive?.papanAktifChannelId || "";
  const previewMember = guild?.members?.cache?.find?.((member) => member?.user && !member.user.bot) || guild?.members?.cache?.first?.();
  const previewUserId = previewMember?.id || "123456789012345678";
  const previewUsername = previewMember?.user?.username || "warga.tulus";
  const previewDisplayName = previewMember?.displayName || "Warga Tulus";
  ...
  return {
    ...
    avatar: previewMember?.displayAvatarURL?.() || "https://cdn.discordapp.com/embed/avatars/0.png",
    joinedAt: previewMember?.joinedAt?.toLocaleDateString?.("id-ID") || "16 Juni 2026",
    server: guild?.name || cfg.serverName || "DESA TULUS",
    serverName: guild?.name || cfg.serverName || "DESA TULUS",
    memberCount: String(guild?.memberCount || "1.234"),
    ownerName: cfg.ownerName || "Pak RW",
    botName: client?.user?.username || cfg.botName || "Pak RW",
    prefix: cfg.prefix || "rw",
    memberRole: roleMention(memberRoleId, "@Member Tulus belum diatur"),
    memberTulusRole: roleMention(memberRoleId, "@Member Tulus belum diatur"),
    staffRole: roleMention(cfg.staffRoleId || "", "@Staff belum diatur"),
    adminRole: roleMention(cfg.adminRoleId || "", "@Admin belum diatur"),
    moderatorRole: roleMention(cfg.moderatorRoleId || "", "@Moderator belum diatur"),
    motmRole: roleMention(cfg.topActive?.memberOfTheMonthRoleId || cfg.level100RoleId || "", "@MOTM belum diatur"),
    donaturRole: roleMention(cfg.donaturRoleId || "", "@Donatur belum diatur"),
    juraganRole: roleMention(cfg.juragan?.roleId || "", "@Juragan belum diatur"),
    rulesChannel: channelMention(cfg.rulesChannelId || "", "#aturan-desa belum diatur"),
    chatWargaChannel: channelMention(cfg.chatWargaChannelId || "", "#chat-warga belum diatur"),
    ticketChannel: channelMention(cfg.ticketChannelId || "", "#ticket belum diatur"),
    aiChannel: channelMention(cfg.aiChannelId || "", "#ai-pak-rw belum diatur"),
    curhatChannel: channelMention(cfg.curhatChannelId || "", "#curhat belum diatur"),
    anonymousCurhatChannel: channelMention(cfg.anonymousCurhatChannelId || "", "#curhat-anonim belum diatur"),
    suggestionChannel: channelMention(cfg.suggestionChannelId || "", "#kotak-saran belum diatur"),
    levelChannel: channelMention(cfg.levelChannelId || "", "#level belum diatur"),
    cekPoinChannel: channelMention(cfg.cekPoinChannelId || "", "#cek-poin belum diatur"),
    topActiveChannel: channelMention(cfg.topActive?.channelId || "", "#top-aktif belum diatur"),
    leaderboardChannel: channelMention(leaderboardId, "#leaderboard-aktif belum diatur"),
    mabarChannel: channelMention(cfg.mabar?.channelId || "", "#cari-mabar belum diatur"),
    boostPoinChannel: channelMention(cfg.boostPoin?.channelId || cfg.juragan?.boostChannelId || "", "#boost-poin belum diatur"),
    welcomeChannel: channelMention(cfg.welcomeChannelId || "", "#welcome belum diatur"),
    level: "37",
    rank: "Warga Rukun",
    total: "18.500",
    chat: "9.900",
    voice: "8.600",
    lifetimeTotal: "100.001",
    cyclePoints: "1",
    motmThreshold: String(cfg.topActive?.pointsThreshold || cfg.level?.cycleResetAtPoints || 100000),
    nextLevel: "38",
    remainingPoints: "500",
    eventName: "Ronda Aktif",
    multiplier: "x1.15",
    duration: "2 jam",
    channels: "chat-warga, voice",
    endsAt: "22.00 WIB",
    by: "Pak RW",
    status: "Aktif",
    month: getMonthLabel ? getMonthLabel() : "JUNI 2026",
    year: "2026",
    date: "16 Juni 2026",
    time: "00.00 WIB",
    today: "16 Juni 2026",
    now: "16 Juni 2026 00.00 WIB"
  };
}
...
function pakRwDiscordPreview(embedCfg = {}, cfg = readConfigFile()) {
  ...
  const unsafeTitleMention = /<@!?\d+>|<@&\d+>|<#\d+>/.test(title);
  return `<div class="discord-preview">
    <div class="discord-label">Preview Discord</div>
    ${content ? `<div class="discord-content">${pakRwPreviewRichText(content, guild)}</div>` : ""}
    <div class="discord-embed" style="border-left-color:${escapeHtml(color)}">
      <div class="discord-row">
        <div class="discord-main">
          ...
          ${unsafeTitleMention ? `<div class="discord-title-warning">Title tidak mendukung mention. Gunakan {displayName}, nama channel, atau nama role.</div>` : ""}
          ${desc ? `<div class="discord-desc">${pakRwPreviewRichText(desc, guild)}</div>` : `<div class="discord-desc muted">Description belum diisi.</div>`}
          ${fields.length ? `<div class="discord-fields">${fields.map((f) => `<div><b>${escapeHtml(pakRwTemplate(f.name || "Field", data))}</b><span>${pakRwPreviewRichText(pakRwTemplate(f.value || "-", data), guild)}</span></div>`).join("")}</div>` : ""}
...
function pakRwPlaceholderPanel(guild = null) {
  return `<aside class="placeholder-panel">
    <div class="panel-title"><b>Placeholder Library</b><span>Klik untuk copy, atau gunakan Discord Data Picker di bawah supaya bisa memilih channel, role, user, server, dan bot langsung dari Discord.</span></div>
    ...
    ${pakRwDiscordPickerPanel(guild)}
  </aside>`;
}

function renderPakRwBindingCenter(req, slug, opts = {}) {
  const cfg = readConfigFile();
  const guild = typeof getDashboardGuild === "function" ? getDashboardGuild() : client.guilds.cache.first();
  const type = slug === "role-manager" ? "role" : "channel";
  const title = type === "channel" ? "Channel Placeholder Center" : "Role Placeholder Center";
  const icon = type === "channel" ? "#️⃣" : "🎭";
  const fields = pakRwBindingFields(type, cfg, guild);
  const status = pakRwBindingStatus(type, cfg, guild);
  const guildWarning = guild ? "" : `<div class="alert danger">Bot belum membaca server Discord. Pastikan bot online dan GUILD_ID benar, lalu refresh dashboard.</div>`;
  return pakRwPremiumShell(`
    ${opts.saved ? `<div class="alert">✅ Semua pemetaan placeholder ${type === "channel" ? "channel" : "role"} berhasil disimpan.</div>` : ""}
    ${opts.error ? `<div class="alert danger">❌ ${escapeHtml(opts.error)}</div>` : ""}
    <section class="hero" style="min-height:220px"><div class="hero-content"><div class="badge-row"><span class="badge ok">Live Discord Aktif</span><span class="badge">Tanpa copy ID</span><span class="badge">Simpan otomatis sebagai ID</span></div><h2>${icon} ${title}</h2><p><b>Caranya hanya tiga langkah:</b> klik dropdown, pilih nama ${type === "channel" ? "channel" : "role"}, lalu tekan Simpan Semua Pilihan. Nama mudah dibaca, tetapi config tetap menyimpan ID Discord yang benar.</p></div></section>
    ${guildWarning}
    <div class="workspace"><form class="form-card" method="post" action="/dashboard/manage/${escapeHtml(slug)}"><h3>1. Pilih ${type === "channel" ? "Channel" : "Role"} untuk Setiap Placeholder</h3><p class="muted">Dropdown di bawah memuat data langsung dari Discord. Setelah memilih semuanya, tekan tombol hijau di bawah.</p><div class="discord-live-status" data-discord-live-status>⏳ Memuat daftar dari Discord...</div><div class="form-grid">${fields}</div><div class="action-bar"><button class="btn" type="submit">💾 2. Simpan Semua Pilihan</button><a class="btn secondary" href="/dashboard/manage/embed">Buka Embed Manager</a><a class="btn ghost" href="/dashboard">Kembali</a></div></form><aside class="preview-card"><div class="panel-title"><b>Status Placeholder</b><span>Nama yang tampil di dashboard, ID tetap disimpan di config.</span></div>${status}${pakRwDiscordPickerPanel(guild)}</aside></div>
  `, slug, title);
}
...
function pakRwPremiumShell(content, active = "dashboard", title = "Pak RW Control Center") {
  ...
  const nav = [
    ...
    ["channel-manager", "#️⃣", "Channel Picker", "/dashboard/manage/channel-manager"],
    ["role-manager", "🎭", "Role Picker", "/dashboard/manage/role-manager"],
  ...
  return `<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;min-height:100vh;color:var(--v-text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,Segoe UI,Arial,sans-serif;background:linear-gradient(180deg,#e9d9b7 0%,#d5e3c3 36%,#cad7e8 68%,#f4ecd9 100%);overflow-x:hidden}a{color:inherit;text-decoration:none}button,input,textarea,select{font:inherit}.village-bg{position:fixed;inset:0;z-index:-3;background:radial-gradient(circle at 12% 12%,rgba(255,232,176,.88),transparent 20%),radial-gradient(circle at 72% 14%,rgba(232,121,182,.22),transparent 24%),linear-gradient(180deg,rgba(105,153,190,.56),rgba(255,255,255,0) 39%),linear-gradient(135deg,#f0d7a5,#bdd6b2 44%,#a4bfd8 100%)}.village-bg:before{content:"";position:absolute;left:-8%;right:-8%;bottom:0;height:52%;background:linear-gradient(180deg,transparent 0%,rgba(40,86,62,.11) 18%,rgba(41,83,58,.24) 60%,rgba(27,58,42,.30) 100%),radial-gradient(ellipse at 20% 100%,rgba(67,113,70,.65) 0 18%,transparent 19%),radial-gradient(ellipse at 51% 95%,rgba(82,133,80,.58) 0 20%,transparent 21%),radial-gradient(ellipse at 81% 100%,rgba(48,93,69,.52) 0 17%,transparent 18%);filter:saturate(.9)}.village-bg:after{content:"";position:absolute;inset:0;background:linear-gradient(110deg,transparent 0 35%,rgba(255,255,255,.20) 48%,transparent 62%),repeating-linear-gradient(90deg,rgba(255,255,255,.03) 0 1px,transparent 1px 7px);mix-blend-mode:soft-light}.floating-grain{position:fixed;inset:0;z-index:-2;pointer-events:none;background-image:radial-gradient(rgba(255,255,255,.38) 1px, transparent 1px);background-size:42px 42px;opacity:.22;animation:floatGrain 18s linear infinite}@keyframes floatGrain{to{transform:translate3d(-42px,42px,0)}}.app{display:grid;grid-template-columns:300px minmax(0,1fr);min-height:100vh}.side{position:sticky;top:0;height:100vh;padding:20px;border-right:1px solid rgba(255,255,255,.36);background:linear-gradient(180deg,rgba(255,255,255,.58),rgba(255,255,255,.30));backdrop-filter:blur(28px);box-shadow:20px 0 80px rgba(26,45,70,.08);overflow:auto}.brand{display:flex;gap:13px;align-items:center;padding:12px;margin-bottom:18px;border:1px solid rgba(255,255,255,.45);border-radius:24px;background:rgba(255,255,255,.42)}.brand-logo{width:52px;height:52px;border-radius:18px;display:grid;place-items:center;background:linear-gradient(135deg,var(--v-green),var(--v-gold));box-shadow:0 18px 48px rgba(95,141,90,.24);font-size:27px}.brand h1{font-size:18px;margin:0}.brand p{margin:4px 0 0;color:var(--v-muted);font-size:12px;line-height:1.35}.nav{display:flex;flex-direction:column;gap:8px}.nav a{display:flex;align-items:center;gap:10px;padding:13px 14px;border-radius:18px;color:rgba(22,32,51,.70);font-weight:850;border:1px solid transparent}.nav a:hover,.nav a.active{background:rgba(255,255,255,.58);border-color:rgba(255,255,255,.56);color:var(--v-text);box-shadow:0 12px 30px rgba(26,45,70,.08)}.main{padding:28px 32px 90px;max-width:1720px;width:100%;margin:0 auto}.top{display:flex;justify-content:space-between;gap:18px;align-items:center;margin-bottom:24px}.crumb{display:flex;flex-direction:column;gap:4px}.crumb b{font-size:20px}.crumb span{color:var(--v-muted);font-size:13px}.top-actions{display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end}.btn{border:0;border-radius:17px;min-height:46px;padding:13px 18px;display:inline-flex;align-items:center;justify-content:center;gap:8px;font-weight:950;background:linear-gradient(135deg,var(--v-green),var(--v-blue));color:white;cursor:pointer;box-shadow:0 16px 34px rgba(73,121,127,.18)}.btn.secondary{background:rgba(255,255,255,.60);color:var(--v-text);border:1px solid rgba(255,255,255,.58);box-shadow:none}.btn.ghost{background:transparent;color:var(--v-muted);border:1px solid rgba(22,32,51,.12);box-shadow:none}.hero{position:relative;overflow:hidden;border-radius:38px;padding:34px;border:1px solid rgba(255,255,255,.56);background:linear-gradient(135deg,rgba(255,255,255,.68),rgba(255,255,255,.32)),radial-gradient(circle at 78% 18%,rgba(255,214,117,.32),transparent 28%),radial-gradient(circle at 10% 90%,rgba(95,141,90,.23),transparent 35%);box-shadow:var(--v-shadow);min-height:286px}.hero:before{content:"";position:absolute;left:0;right:0;bottom:0;height:48%;background:linear-gradient(180deg,transparent,rgba(95,141,90,.18)),repeating-linear-gradient(170deg,rgba(75,129,72,.18) 0 2px,transparent 2px 28px);clip-path:polygon(0 58%,15% 45%,33% 60%,51% 38%,70% 55%,86% 44%,100% 58%,100% 100%,0 100%)}.hero:after{content:"🏠";position:absolute;right:42px;bottom:38px;font-size:104px;opacity:.18;filter:blur(.2px)}.hero-content{position:relative;z-index:1;max-width:900px}.badge-row{display:flex;flex-wrap:wrap;gap:10px;margin:18px 0}.badge{border:1px solid rgba(22,32,51,.10);background:rgba(255,255,255,.58);border-radius:999px;padding:9px 12px;color:rgba(22,32,51,.72);font-weight:850;font-size:13px}.badge.ok{color:#2e6840;background:rgba(119,184,112,.18);border-color:rgba(56,126,64,.16)}.badge.warn{color:#8a641c;background:rgba(231,189,104,.22);border-color:rgba(138,100,28,.18)}.hero h2{font-size:48px;letter-spacing:-1.8px;line-height:1.02;margin:0}.hero p{font-size:16px;line-height:1.7;color:var(--v-muted);max-width:780px}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:18px;margin:22px 0}.stat{border:1px solid rgba(255,255,255,.56);background:linear-gradient(180deg,rgba(255,255,255,.66),rgba(255,255,255,.36));border-radius:26px;padding:22px;box-shadow:0 18px 55px rgba(26,45,70,.10)}.stat span{font-size:12px;font-weight:950;letter-spacing:.8px;text-transform:uppercase;color:var(--v-soft)}.stat b{display:block;font-size:29px;margin:8px 0 4px}.stat small{color:var(--v-muted);line-height:1.5}.section{border:1px solid rgba(255,255,255,.56);background:linear-gradient(180deg,rgba(255,255,255,.66),rgba(255,255,255,.36));border-radius:32px;padding:24px;margin:22px 0;box-shadow:0 18px 60px rgba(26,45,70,.09)}.section-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:18px}.section h3{margin:0;font-size:24px}.muted{color:var(--v-muted);line-height:1.62}.plugin-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}.plugin{position:relative;min-height:218px;display:flex;flex-direction:column;gap:10px;border:1px solid rgba(22,32,51,.10);background:linear-gradient(180deg,rgba(255,255,255,.62),rgba(255,255,255,.32));border-radius:26px;padding:19px;transition:.18s ease;overflow:hidden}.plugin:hover{transform:translateY(-4px);box-shadow:0 22px 65px rgba(26,45,70,.14);border-color:rgba(95,141,90,.24)}.plugin:after{content:"";position:absolute;right:-32px;top:-32px;width:110px;height:110px;border-radius:50%;background:radial-gradient(circle,rgba(109,157,247,.16),transparent 70%)}.plug-icon{width:54px;height:54px;border-radius:19px;display:grid;place-items:center;background:linear-gradient(135deg,rgba(95,141,90,.18),rgba(231,189,104,.23));font-size:27px}.plugin b{font-size:17px}.plugin p{margin:0;color:var(--v-muted);line-height:1.55;font-size:13.5px;flex:1}.plug-foot{display:flex;gap:8px;flex-wrap:wrap;align-items:center}.mini{border:1px solid rgba(22,32,51,.10);background:rgba(255,255,255,.48);border-radius:999px;padding:7px 10px;color:rgba(22,32,51,.65);font-weight:850;font-size:12px}.dot{width:10px;height:10px;border-radius:99px;background:var(--v-leaf);box-shadow:0 0 0 4px rgba(126,183,123,.16)}.dot.off{background:#b58a4c}.workspace{display:grid;grid-template-columns:minmax(0,1.18fr) minmax(370px,.82fr);gap:22px;align-items:start}.form-card,.preview-card{border:1px solid rgba(255,255,255,.56);background:linear-gradient(180deg,rgba(255,255,255,.68),rgba(255,255,255,.38));border-radius:32px;padding:24px;box-shadow:0 18px 60px rgba(26,45,70,.09)}.preview-card{position:sticky;top:20px}.form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:15px}.prw-field{display:flex;flex-direction:column;gap:8px}.prw-field.wide{grid-column:1/-1}.prw-field span{font-weight:900;color:rgba(22,32,51,.78);font-size:13px}.prw-field small{font-size:12px;color:var(--v-muted);line-height:1.45}.prw-field input,.prw-field textarea,.prw-field select{width:100%;border:1px solid rgba(22,32,51,.12);background:rgba(255,255,255,.66);border-radius:18px;padding:14px 14px;color:var(--v-text);outline:none}.prw-field textarea{min-height:170px;line-height:1.6}.prw-field input:focus,.prw-field textarea:focus,.prw-field select:focus{border-color:rgba(109,157,247,.50);box-shadow:0 0 0 4px rgba(109,157,247,.12)}.prw-switch{display:grid;grid-template-columns:56px 1fr;gap:12px;align-items:center;border:1px solid rgba(22,32,51,.10);background:rgba(255,255,255,.48);padding:13px;border-radius:20px}.prw-switch input{display:none}.prw-switch span{width:52px;height:30px;border-radius:999px;background:rgba(22,32,51,.16);position:relative}.prw-switch span:after{content:"";position:absolute;width:24px;height:24px;border-radius:50%;left:3px;top:3px;background:white;box-shadow:0 4px 10px rgba(0,0,0,.18);transition:.17s}.prw-switch input:checked+span{background:linear-gradient(135deg,var(--v-green),var(--v-blue))}.prw-switch input:checked+span:after{left:25px}.prw-switch b{font-size:14px}.prw-switch small{grid-column:2;color:var(--v-muted);font-size:12px}.discord-preview{background:#313338;border-radius:20px;padding:18px;color:#dbdee1;box-shadow:0 18px 60px rgba(0,0,0,.16)}.discord-label{color:#949ba4;font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:.8px;margin-bottom:12px}.discord-content{margin-bottom:10px;line-height:1.45;white-space:pre-wrap}.discord-embed{background:#2b2d31;border-left:5px solid #7dbd77;border-radius:8px;padding:14px;max-width:680px}.discord-row{display:flex;gap:16px}.discord-main{min-width:0;flex:1}.discord-author{font-size:13px;font-weight:800;color:#fff;margin-bottom:7px}.discord-title{font-weight:900;color:#9fc5ff;margin-bottom:8px}.discord-desc{font-size:13.5px;line-height:1.55;color:#dbdee1;white-space:normal}.discord-desc.muted{color:#949ba4}.discord-footer{font-size:12px;color:#b5bac1;margin-top:12px}.discord-thumb{width:82px;height:82px;border-radius:12px;object-fit:cover}.discord-image{display:block;width:100%;max-height:280px;object-fit:cover;border-radius:12px;margin-top:14px}.discord-image.empty{border:1px dashed rgba(255,255,255,.18);min-height:88px;display:grid;place-items:center;color:#949ba4;text-align:center;padding:14px}.discord-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:10px}.discord-fields div{background:rgba(255,255,255,.035);border-radius:8px;padding:8px}.discord-fields b,.discord-fields span{display:block;font-size:12px}.placeholder-panel{border:1px solid rgba(22,32,51,.10);background:rgba(255,255,255,.48);border-radius:24px;padding:16px;margin-top:16px}.panel-title b,.panel-title span{display:block}.panel-title span{font-size:12px;color:var(--v-muted);line-height:1.5;margin-top:4px}.ph-group{margin-top:13px}.ph-group h4{margin:0 0 8px;font-size:12px;color:var(--v-soft);text-transform:uppercase;letter-spacing:.7px}.ph-group div{display:flex;gap:7px;flex-wrap:wrap}.ph-chip{border:1px solid rgba(22,32,51,.10);background:rgba(255,255,255,.58);border-radius:999px;padding:7px 10px;color:rgba(22,32,51,.72);font-weight:850;font-size:12px;cursor:pointer}.discord-data-picker{margin-top:18px;border:1px solid rgba(22,32,51,.12);background:linear-gradient(180deg,rgba(255,255,255,.62),rgba(255,255,255,.42));border-radius:22px;padding:16px}.picker-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:11px;margin-top:14px}.picker-grid label{display:flex;flex-direction:column;gap:7px}.picker-grid label>span{font-size:12px;font-weight:900;color:rgba(22,32,51,.72)}.picker-grid input,.picker-grid select{width:100%;border:1px solid rgba(22,32,51,.12);background:rgba(255,255,255,.76);border-radius:15px;padding:11px 12px;color:var(--v-text);outline:none}.picker-grid select[size]{min-height:190px;padding:8px}.picker-wide{grid-column:1/-1}.picker-active,.picker-result{margin-top:11px;border-radius:14px;padding:10px 12px;font-size:12px;line-height:1.45}.picker-active{background:rgba(109,157,247,.12);color:#365b9e;border:1px solid rgba(109,157,247,.20)}.picker-result{background:rgba(126,183,123,.14);color:#2e6840;border:1px solid rgba(95,141,90,.20);word-break:break-word}.picker-links{display:flex;gap:10px;flex-wrap:wrap;margin-top:12px}.picker-links a{font-size:12px;font-weight:900;color:#365b9e;border-bottom:1px dashed rgba(54,91,158,.35)}.picker-live-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.picker-live-head .btn{min-height:40px;padding:10px 13px;font-size:12px}.discord-live-status{margin:12px 0;border:1px solid rgba(109,157,247,.24);background:rgba(109,157,247,.12);color:#365b9e;border-radius:15px;padding:11px 13px;font-size:12px;font-weight:900;line-height:1.45}.discord-live-status.live-ok{border-color:rgba(95,141,90,.25);background:rgba(126,183,123,.16);color:#2e6840}.discord-live-status.live-error{border-color:rgba(200,82,82,.24);background:rgba(228,94,94,.14);color:#8a2b2b}.picker-steps{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin:12px 0}.picker-steps span{display:flex;align-items:center;gap:7px;border:1px solid rgba(22,32,51,.10);background:rgba(255,255,255,.54);border-radius:13px;padding:9px;font-size:11px;font-weight:850;color:rgba(22,32,51,.68)}.picker-steps b{width:22px;height:22px;display:grid;place-items:center;border-radius:50%;background:linear-gradient(135deg,var(--v-green),var(--v-blue));color:#fff}.discord-live-field select{min-height:56px;font-weight:850;border-width:2px}.discord-live-field select:focus{border-color:var(--v-green)!important}.discord-live-field small b{color:#2e6840}.discord-live-field select[data-live-ready="false"]{opacity:.72}.discord-live-field select option{font-weight:700}.discord-live-field select optgroup{font-weight:950}.binding-status{display:flex;flex-direction:column;gap:9px;margin-top:14px}.binding-row{display:grid;grid-template-columns:minmax(150px,.9fr) minmax(0,1.2fr) auto;gap:10px;align-items:center;border:1px solid rgba(22,32,51,.10);background:rgba(255,255,255,.48);border-radius:16px;padding:11px}.binding-row code{font-size:12px;color:#5b4f8f;word-break:break-word}.binding-row span{font-size:12px;color:var(--v-muted);word-break:break-word}.binding-row b{font-size:11px;border-radius:999px;padding:6px 9px}.binding-row b.valid{background:rgba(126,183,123,.20);color:#2e6840}.binding-row b.invalid{background:rgba(231,189,104,.22);color:#7a5519}.discord-mention{display:inline-block;background:rgba(88,101,242,.30);color:#c9cdfb;border-radius:3px;padding:0 2px;font-weight:700}.discord-title-warning{margin:6px 0 9px;color:#f0b66c;font-size:11px;line-height:1.4}.warn-box{margin-top:13px;border:1px solid rgba(181,138,76,.24);background:rgba(231,189,104,.18);color:#6b4c16;border-radius:16px;padding:11px;font-size:12px;line-height:1.45}.alert{border:1px solid rgba(95,141,90,.22);background:rgba(126,183,123,.18);color:#2e6840;border-radius:18px;padding:13px 14px;margin:14px 0;font-weight:850}.alert.danger{border-color:rgba(200,82,82,.24);background:rgba(228,94,94,.14);color:#8a2b2b}.action-bar{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px}.embed-list{display:grid;grid-template-columns:300px minmax(0,1fr);gap:18px}.template-list{display:flex;flex-direction:column;gap:8px;max-height:720px;overflow:auto}.template-list a{border:1px solid rgba(22,32,51,.10);background:rgba(255,255,255,.48);border-radius:17px;padding:12px;font-weight:850}.template-list a.active{background:linear-gradient(135deg,rgba(95,141,90,.18),rgba(109,157,247,.14));border-color:rgba(95,141,90,.22)}.toast{position:fixed;right:18px;bottom:18px;background:rgba(7,17,31,.94);color:white;border-radius:999px;padding:12px 15px;font-weight:900;opacity:0;transform:translateY(10px);transition:.18s;z-index:99}.toast.show{opacity:1;transform:translateY(0)}.mobile-toggle{display:none;position:fixed;left:14px;bottom:14px;z-index:80;border:0;border-radius:999px;background:var(--v-deeper);color:white;padding:12px 15px;font-weight:950}.helper-strip{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.helper-strip div{border:1px solid rgba(22,32,51,.10);background:rgba(255,255,255,.42);border-radius:20px;padding:15px}.helper-strip b{display:block;margin-bottom:5px}.helper-strip span{font-size:12px;line-height:1.5;color:var(--v-muted)}@media(max-width:1280px){.grid,.plugin-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.workspace,.embed-list{grid-template-columns:1fr}.preview-card{position:relative;top:auto}.helper-strip{grid-template-columns:1fr}}@media(max-width:860px){.picker-live-head{flex-direction:column}.picker-live-head .btn{width:100%}.picker-steps{grid-template-columns:repeat(2,minmax(0,1fr))}.picker-grid{grid-template-columns:1fr}.picker-wide{grid-column:auto}.binding-row{grid-template-columns:1fr}.app{grid-template-columns:1fr}.side{position:fixed;left:0;top:0;bottom:0;width:min(86vw,320px);transform:translateX(-110%);transition:.2s;z-index:70}.nav-open .side{transform:translateX(0)}.mobile-toggle{display:block}.main{padding:18px 14px 80px}.top,.section-head{flex-direction:column;align-items:stretch}.hero{padding:24px;border-radius:28px}.hero h2{font-size:34px}.grid,.plugin-grid,.form-grid{grid-template-columns:1fr}.form-card,.preview-card,.section{border-radius:24px;padding:18px}.discord-row{flex-direction:column}.discord-thumb{width:72px;height:72px}.discord-fields{grid-template-columns:1fr}}
    ...
    const pakRwToast = (text) => { const toast = document.getElementById('toast'); if (!toast) return; toast.textContent = text; toast.classList.add('show'); setTimeout(() => toast.classList.remove('show'), 1400); };
    let pakRwActiveEditor = null;

    document.addEventListener('focusin', (e) => {
      const el = e.target;
      if (!el || !el.matches || !el.matches('textarea, input[type="text"], input[type="url"], input[type="search"], input:not([type])')) return;
      if (!el.closest('form')) return;
      pakRwActiveEditor = el;
      const label = el.closest('label')?.querySelector('span')?.textContent || el.name || 'field teks';
      document.querySelectorAll('[data-picker-active]').forEach((node) => { node.textContent = 'Field aktif: ' + label; });
    });
    ...
    let pakRwActiveDiscordSelect = null;
    let pakRwDiscordLiveData = null;
    let pakRwDiscordLoading = false;

    document.addEventListener('focusin', (e) => {
      const select = e.target?.closest?.('[data-discord-live-select]');
      if (!select) return;
      pakRwActiveDiscordSelect = select;
      const label = select.closest('label')?.querySelector('span')?.textContent || select.name || 'dropdown fitur';
      document.querySelectorAll('[data-picker-active]').forEach((node) => { node.textContent = 'Dropdown aktif: ' + label; });
    });
    ...
    document.addEventListener('change', (e) => {
      ...
      const readable = select.selectedOptions?.[0]?.textContent || 'Belum memilih';
      const helper = select.closest('label')?.querySelector('[data-discord-readable]');
      if (helper) helper.textContent = select.value ? 'Terpilih: ' + readable : 'Belum memilih ' + (select.dataset.discordLiveSelect || 'data');
      document.body.classList.add('has-unsaved');
    });

    const pakRwPickerControllers = [];

    const pakRwSetLiveStatus = (text, state = '') => {
      document.querySelectorAll('[data-discord-live-status]').forEach((node) => {
        node.textContent = text;
        node.classList.toggle('live-ok', state === 'ok');
        node.classList.toggle('live-error', state === 'error');
      });
    };

    const pakRwPopulateLiveSelects = (data) => {
      document.querySelectorAll('[data-discord-live-select]').forEach((select) => {
        const kind = select.dataset.discordLiveSelect || 'channel';
        const selected = String(select.value || select.dataset.selected || '');
        let items = Array.isArray(data?.[kind]) ? data[kind] : [];
        if (kind === 'channel' && select.dataset.channelScope === 'text') {
          items = items.filter((item) => ['Text', 'Announcement', 'Forum'].includes(String(item.meta || '')));
        }

        select.innerHTML = '';
        const placeholder = document.createElement('option');
        placeholder.value = '';
        placeholder.textContent = kind === 'channel' ? '— Pilih nama channel Discord —' : '— Pilih nama role Discord —';
        ...
        if (kind === 'channel') {
          ...
          groups.forEach((groupItems, label) => {
            ...
            group.label = label + ' (' + groupItems.length + ')';
            groupItems.forEach((item) => {
              const option = new Option(item.name || item.id, item.id);
              option.dataset.rawName = item.rawName || item.name || '';
              group.appendChild(option);
            });
            select.appendChild(group);
          });
        } else {
          items.forEach((item) => select.add(new Option(item.name || item.id, item.id)));
        }

        if (selected && !items.some((item) => String(item.id) === selected)) {
          const saved = new Option('Tersimpan sebelumnya • ' + selected, selected, true, true);
          select.appendChild(saved);
        }
        select.value = selected;
        select.dataset.liveReady = 'true';

        const helper = select.closest('label')?.querySelector('[data-discord-readable]');
        const readable = select.selectedOptions?.[0]?.textContent || '';
        if (helper) helper.textContent = selected ? 'Terpilih: ' + readable : 'Belum memilih ' + kind;
      });
    };

    const pakRwSetupPicker = (root) => {
      let data = {};
      try { data = JSON.parse(root.querySelector('.discord-picker-data')?.textContent || '{}'); } catch { data = {}; }
      const typeEl = root.querySelector('[data-picker-type]');
      const searchEl = root.querySelector('[data-picker-search]');
      const itemEl = root.querySelector('[data-picker-item]');
      const formatEl = root.querySelector('[data-picker-format]');
      const placeholderEl = root.querySelector('[data-picker-placeholder]');
      const resultEl = root.querySelector('[data-picker-result]');
      let visibleItems = [];

      const fieldDisallowsMention = () => {
        const name = String(pakRwActiveEditor?.name || '').toLowerCase();
        return name.includes('title') || name.includes('author') || name.includes('footer') || name.includes('buttonlabel');
      };
      const selectedItem = () => visibleItems[Number(itemEl?.value || 0)] || null;
      const formatSelected = () => {
        const item = selectedItem();
        if (!item) return '';
        const mode = formatEl?.value || 'mention';
        if (mode === 'id') return item.id || '';
        if (mode === 'name') return item.name || item.rawName || '';
        if (fieldDisallowsMention()) return item.name || item.rawName || '';
        return item.mention || item.name || item.id || '';
      };
      const updateResult = () => {
        if (!resultEl) return;
        const item = selectedItem();
        const value = formatSelected();
        if (!item || !value) { resultEl.textContent = 'Belum ada data dipilih.'; return; }
        resultEl.textContent = 'Dipilih: ' + (item.name || item.id) + ' • ID: ' + (item.id || '-') + ' • Hasil: ' + value;
      };
      const renderPlaceholderOptions = () => {
        if (!placeholderEl) return;
        placeholderEl.innerHTML = '';
        const list = data.placeholders?.[typeEl?.value || 'channel'] || [];
        list.forEach((value) => placeholderEl.add(new Option(value, value)));
        if (!list.length) placeholderEl.add(new Option('Tidak ada placeholder', ''));
      };
      const renderItems = () => {
        if (!itemEl) return;
        const q = String(searchEl?.value || '').trim().toLowerCase();
        const source = Array.isArray(data[typeEl?.value || 'channel']) ? data[typeEl?.value || 'channel'] : [];
        visibleItems = source.filter((item) => !q || String(item.name || '').toLowerCase().includes(q) || String(item.rawName || '').toLowerCase().includes(q) || String(item.id || '').includes(q) || String(item.meta || '').toLowerCase().includes(q)).slice(0, 1000);
        itemEl.innerHTML = '';
        visibleItems.forEach((item, index) => itemEl.add(new Option((item.name || item.id) + (item.meta ? ' • ' + item.meta : ''), String(index))));
        if (!visibleItems.length) itemEl.add(new Option('Belum ada data. Tekan Muat Ulang Discord.', ''));
        updateResult();
      };
      const setData = (next) => { data = next || {}; renderPlaceholderOptions(); renderItems(); };

      typeEl?.addEventListener('change', () => { if (searchEl) searchEl.value = ''; renderPlaceholderOptions(); renderItems(); });
      searchEl?.addEventListener('input', renderItems);
      itemEl?.addEventListener('change', updateResult);
      itemEl?.addEventListener('dblclick', () => root.querySelector('[data-picker-use-select]')?.click());
      formatEl?.addEventListener('change', updateResult);

      root.querySelector('[data-picker-use-select]')?.addEventListener('click', () => {
        const item = selectedItem();
        const type = typeEl?.value || 'channel';
        if (!item) return pakRwToast('Pilih nama Discord dulu.');
        const candidates = [...document.querySelectorAll('[data-discord-live-select="' + type + '"]')];
        const target = pakRwActiveDiscordSelect?.dataset?.discordLiveSelect === type ? pakRwActiveDiscordSelect : (candidates.length === 1 ? candidates[0] : null);
        if (!target) return pakRwToast('Klik dropdown ' + (type === 'channel' ? 'channel' : 'role') + ' tujuan dulu.');
        target.value = item.id || '';
        target.dataset.selected = item.id || '';
        target.dispatchEvent(new Event('change', { bubbles: true }));
        pakRwToast('Dipilih: ' + (item.name || item.id));
      });

      root.querySelector('[data-picker-insert]')?.addEventListener('click', () => {
        const target = pakRwActiveEditor || document.querySelector('textarea[name="description"], input[name="content"], textarea');
        const value = formatSelected();
        if (!value) return pakRwToast('Pilih data Discord dulu.');
        if (!target) return pakRwToast('Klik Content atau Description dulu.');
        if (pakRwInsertAtCaret(target, value)) pakRwToast('Tag Discord dimasukkan.');
      });
      root.querySelector('[data-picker-placeholder-insert]')?.addEventListener('click', () => {
        const target = pakRwActiveEditor || document.querySelector('textarea[name="description"], input[name="content"], textarea');
        const value = placeholderEl?.value || '';
        if (!value) return pakRwToast('Pilih placeholder dulu.');
        if (!target) return pakRwToast('Klik Content atau Description dulu.');
        if (pakRwInsertAtCaret(target, value)) pakRwToast('Placeholder dimasukkan: ' + value);
      });
      root.querySelector('[data-picker-copy]')?.addEventListener('click', async () => {
        const value = formatSelected() || placeholderEl?.value || '';
        if (!value) return pakRwToast('Belum ada hasil yang bisa disalin.');
        try { await navigator.clipboard.writeText(value); } catch {}
        pakRwToast('Disalin: ' + value);
      });
      root.querySelector('[data-discord-refresh]')?.addEventListener('click', () => pakRwLoadDiscordData(true));

      setData(data);
      return { setData };
    };

    document.querySelectorAll('.discord-data-picker').forEach((root) => pakRwPickerControllers.push(pakRwSetupPicker(root)));

    async function pakRwLoadDiscordData(force = false) {
      if (pakRwDiscordLoading) return;
      pakRwDiscordLoading = true;
      pakRwSetLiveStatus('⏳ Memuat channel dan role langsung dari Discord...');
      document.querySelectorAll('[data-discord-refresh]').forEach((btn) => { btn.disabled = true; btn.textContent = 'Memuat...'; });
      try {
        const response = await fetch('/api/discord-picker-data?refresh=' + (force ? Date.now() : '0'), { cache: 'no-store', headers: { Accept: 'application/json' } });
        const payload = await response.json().catch(() => ({}));
        if (!response.ok || !payload.ok) throw new Error(payload.error || 'Dashboard belum terhubung ke bot Discord.');
        pakRwDiscordLiveData = payload;
        pakRwPopulateLiveSelects(payload);
        pakRwPickerControllers.forEach((controller) => controller.setData(payload));
        const channelCount = payload.counts?.channels ?? payload.channel?.length ?? 0;
        const roleCount = payload.counts?.roles ?? payload.role?.length ?? 0;
        pakRwSetLiveStatus('✅ Terhubung ke ' + (payload.guild?.name || 'DESA TULUS') + ' • ' + channelCount + ' channel • ' + roleCount + ' role', 'ok');
      } catch (error) {
        pakRwSetLiveStatus('❌ ' + (error?.message || 'Gagal memuat Discord') + ' Pastikan bot online, GUILD_ID benar, lalu tekan Muat Ulang Discord.', 'error');
      } finally {
        pakRwDiscordLoading = false;
        document.querySelectorAll('[data-discord-refresh]').forEach((btn) => { btn.disabled = false; btn.textContent = '↻ Muat Ulang Discord'; });
      }
    }

    pakRwLoadDiscordData(false);
  </script></body></html>`;
}
...
function renderPakRwManagePage(req, slug, opts = {}) {
  ...
  if (slug === "channel-manager" || slug === "role-manager") return renderPakRwBindingCenter(req, slug, opts);
...
function savePakRwManagePage(req, slug) {
  ...
  if (slug === "channel-manager" || slug === "role-manager") {
    const bindings = slug === "channel-manager" ? PAKRW_CHANNEL_PLACEHOLDER_BINDINGS : PAKRW_ROLE_PLACEHOLDER_BINDINGS;
    for (const binding of bindings) {
      if (req.body[binding.field] === undefined) continue;
      pakRwSetByPath(cfg, binding.path, pakRwSafeId(req.body[binding.field]));
    }

    if (slug === "channel-manager") {
      const leaderboardId = pakRwGetByPath(cfg, "papanAktif.channelId", "");
      cfg.papanAktif = cfg.papanAktif || {};
      cfg.leaderboardAktif = { ...(cfg.leaderboardAktif || {}), ...cfg.papanAktif, channelId: leaderboardId };
      cfg.topActive = cfg.topActive || {};
      cfg.topActive.leaderboardActiveChannelId = leaderboardId;
      cfg.topActive.useOneChannel = false;
    } else {
      const memberRoleId = pakRwGetByPath(cfg, "memberTulusRoleId", "");
      cfg.welcome = cfg.welcome || {};
      cfg.welcome.memberRoleId = memberRoleId;
      const motmRoleId = pakRwGetByPath(cfg, "topActive.memberOfTheMonthRoleId", "");
      if (motmRoleId) cfg.level100RoleId = motmRoleId;
    }
    ...
    cfg.dashboard.discordPickerEnabled = true;
    cfg.dashboard.placeholderBindingCenterEnabled = true;
    writeConfigFile(cfg);
    return;
  }
...
app.post("/embeds", requireDashboardAuth, (req, res) => {
  try {
    ...
    cfg.embeds.dashboard = cfg.embeds.dashboard || {};
    cfg.embeds.dashboard.brandTitle = "Pak RW v10.10.63";
...
app.get("/api/discord-picker-data", requireDashboardAuth, async (req, res) => {
  res.set("Cache-Control", "no-store, no-cache, must-revalidate, proxy-revalidate");
  try {
    if (!client.isReady()) {
      return res.status(503).json({ ok: false, error: "Bot Pak RW belum selesai login ke Discord." });
    }

    const guild = getDashboardGuild();
    if (!guild) {
      return res.status(503).json({ ok: false, error: "Server DESA TULUS belum ditemukan. Periksa GUILD_ID dan pastikan bot sudah masuk server." });
    }
    ...
    return res.json({
      ...
      counts: {
        channels: payload.channel.length,
        roles: payload.role.length,
        users: payload.user.length
      },
      ...payload
    });
  } catch (err) {
    console.log("DISCORD PICKER API ERROR:", err.message);
    return res.status(500).json({ ok: false, error: `Gagal membaca data Discord: ${err.message}` });
  }
});
...
function buildBoostPoinPublicPayload(guild, state = "active") {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `<a:Desa_Tulus2:1518502350363430932> DESA TULUS | Boost Poin` })
    .setTimestamp();
...
function buildLevelUpEmbed(member, userData) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setTitle(applyTemplate(e.title || "<a:Desa_Tulus2:1518502350363430932> WARGA NAIK LEVEL!", data))
...
function getTopActiveConfig() {
  ...
  return {
    ...
    leaderboardActiveFooter: t.leaderboardActiveFooter || "<a:Desa_Tulus2:1518502350363430932> DESA TULUS | Leaderboard Aktif Warga",
...
function getTopActiveArrowEmoji() {
  return String(getTopActiveConfig().rowArrowEmoji || "<a:Desa_Tulus2:1518502350363430932>").trim();
}
...
function applyTemplate(text = "", data = {}) {
  ...
  const defaults = {
    ...
    leaderboardChannel: channelTag(config.papanAktif?.channelId || config.leaderboardAktif?.channelId || config.topActive?.leaderboardActiveChannelId || config.topActive?.papanAktifChannelId, "#leaderboard-aktif"),
    ...
    welcomeChannel: channelTag(config.welcomeChannelId, "#welcome"),
...
client.on(Events.GuildMemberAdd, async (member) => {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(description)
      .setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 512 }))
      .setFooter({ text: makeOTFooter("<a:Desa_Tulus2:1518502350363430932> DESA TULUS | Tempat warga baik berkumpul") })
      .setTimestamp();
