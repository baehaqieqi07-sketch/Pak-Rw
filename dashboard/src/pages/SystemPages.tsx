...
import { Activity, Archive, CheckCircle2, CircleAlert, Copy, DatabaseBackup, ExternalLink, FileImage, Hash, Headphones, IdCard, LoaderCircle, RefreshCcw, Save, Search, Server, Settings, Shield, ShieldCheck, TerminalSquare, Upload, Users } from "lucide-react";
import type { DashboardHealth } from "../app/types";
...
export function CommandCenterPage() {
  ...
  const commands = [
    ["rwhelp", "Core", "Daftar bantuan"], ["rwtanya", "AI", "Tanya Pak RW"], ["rwcurhat", "Community", "Panel curhat"],
    ["rwcekpoin", "Level", "Cek poin"], ["rwlevel", "Level", "Profil level"], ["rwrank", "Level", "Peringkat warga"],
    ["rwtopaktif", "Leaderboard", "Top aktif bulanan"], ["rwpapanaktif", "Leaderboard", "Papan aktif lifetime"],
    ["rwleaderboardaktif", "Leaderboard", "Leaderboard aktif"], ["rwpostpapanaktif", "Leaderboard", "Post papan aktif"],
    ["rwai", "AI", "AI Pak RW"], ["rwfitur", "Core", "Daftar fitur"], ["rwktp", "KTP", "Buat KTP Warga"],
    ["rwktppanel", "KTP", "Kirim panel KTP"], ["rwid", "System", "Ekspor ID server"]
  ];
  const visible = commands.filter((item) => item.join(" ").toLowerCase().includes(query.trim().toLowerCase()));
  return <div className="page-stack page-enter"><PageHeader icon={TerminalSquare} kicker="Administration" title="Command Center" description="Daftar command aktif. Halaman ini read-only karena backend pengaturan command belum tersedia." /><Card><CardHeader title="Command registry" description={`${visible.length} command • prefix ${data.status.prefix}`} action={<span className="readonly-badge">Read-only</span>} /><div className="search-field command-search"><Search size={17}/><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Cari command atau kategori" /></div><div className="command-table command-table-rich">{visible.map(([command, category, description]) => <div key={command}><code>{command}</code><span>{category}</span><small>{description}</small><strong>Active</strong></div>)}</div>{!visible.length?<div className="empty-inline">Command tidak ditemukan.</div>:null}</Card></div>;
}
...
export function LogsPage() {
  ...
  const loadHealth = async () => { setLoading(true); setError(""); try { setHealth(await api.health()); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } finally { setLoading(false); } };
  useEffect(() => { loadHealth(); }, []);
  const activities = (data.activity || []).filter((item) => filter === "all" || `${item.type || ""} ${item.title || ""}`.toLowerCase().includes(filter));
  return <div className="page-stack page-enter"><PageHeader icon={Activity} kicker="System" title="Logs & Health" description="Runtime, API, dan aktivitas dashboard tanpa token atau nilai environment." /><div className="stats-grid system-stats"><Card className="stat-card"><div className="stat-icon"><Server size={20}/></div><div className="stat-copy"><span>Discord</span><strong>{data.status.botOnline?"Online":"Offline"}</strong><small>{data.status.pingMs!=null?`${data.status.pingMs} ms`:data.status.botTag||"Belum terhubung"}</small></div></Card><Card className="stat-card"><div className="stat-icon"><Archive size={20}/></div><div className="stat-copy"><span>Database</span><strong>{data.status.databaseMode}</strong><small>Storage mode</small></div></Card><Card className="stat-card"><div className="stat-icon"><Activity size={20}/></div><div className="stat-copy"><span>Dashboard API</span><strong>{health?.ok?"Healthy":loading?"Checking":"Unavailable"}</strong><small>{health?.checkedAt?new Date(health.checkedAt).toLocaleTimeString("id-ID"):"Health endpoint"}</small></div></Card><Card className="stat-card"><div className="stat-icon"><FileImage size={20}/></div><div className="stat-copy"><span>Assets</span><strong>{health?.assetFoldersReady?"Ready":"Check"}</strong><small>Dashboard & bot assets</small></div></Card></div><Card><CardHeader title="Health checks" description="Pemeriksaan read-only dari proses dashboard." action={<Button variant="secondary" icon={<RefreshCcw className={loading?"spin":""} size={15}/>} onClick={loadHealth} disabled={loading}>Refresh</Button>} />{loading?<div className="empty-inline"><LoaderCircle className="spin" size={18}/>Memeriksa layanan...</div>:error?<div className="inline-warning">{error}</div>:<div className="health-list"><div className={health?.configReadable?"":"warning"}>{health?.configReadable?<CheckCircle2 size={18}/>:<CircleAlert size={18}/>}<span>Config {health?.configReadable?"dapat dibaca":"tidak terbaca"}</span></div><div className={health?.dashboardBuild?"":"warning"}>{health?.dashboardBuild?<CheckCircle2 size={18}/>:<CircleAlert size={18}/>}<span>Production build {health?.dashboardBuild?"tersedia":"belum tersedia"}</span></div><div className={health?.botOnline?"":"warning"}>{health?.botOnline?<CheckCircle2 size={18}/>:<CircleAlert size={18}/>}<span>Discord {health?.botOnline?"online":"offline"}</span></div><div className={health?.assetFoldersReady?"":"warning"}>{health?.assetFoldersReady?<CheckCircle2 size={18}/>:<CircleAlert size={18}/>}<span>Asset folders {health?.assetFoldersReady?"siap":"perlu dicek"}</span></div></div>}</Card><Card><CardHeader title="Activity log" description="Aktivitas dashboard yang aman ditampilkan." /><div className="log-filters">{["all","dashboard","leaderboard","embed","warning","error"].map((value)=><button key={value} className={filter===value?"is-active":""} onClick={()=>setFilter(value)}>{value}</button>)}</div><div className="activity-list">{activities.length?activities.map((item,index)=><div className="activity-item" key={`${item.at}-${index}`}><span className="activity-marker"/><div><strong>{item.title}</strong><small>{item.detail||item.type||"Dashboard"}</small></div><time>{new Date(item.at).toLocaleString("id-ID")}</time></div>):<div className="empty-inline">Tidak ada aktivitas untuk filter ini.</div>}</div></Card></div>;
}

export function BackupPage() {
  return <div className="page-stack page-enter"><PageHeader icon={DatabaseBackup} kicker="System" title="Backup Center" description="Status perlindungan data dan batas kemampuan backup dashboard." /><Card className="backend-unavailable-card"><CardHeader title="Backup backend belum diaktifkan" description="Dashboard belum memiliki endpoint backup/restore yang tervalidasi. Tidak ada tombol palsu atau restore otomatis." action={<span className="readonly-badge">Unavailable</span>} /><div className="info-panel"><CircleAlert size={19}/><p>Backup tetap dilakukan melalui mekanisme hosting atau MongoDB. Restore tidak tersedia dari dashboard dan tidak akan dijalankan tanpa konfirmasi eksplisit.</p></div></Card><Card><CardHeader title="Protected data" description="Bagian ini tidak boleh dihapus atau ditimpa saat update." /><div className="backup-grid">{["MongoDB data","Level dan poin","Papan Aktif lifetime","MOTM","Donatur dan Juragan","Channel dan role IDs","Embed templates","AI memory"].map((item)=><div key={item}><CheckCircle2 size={17}/><span>{item}</span></div>)}</div></Card><Card><CardHeader title="Dashboard rollback" description="Menonaktifkan web dashboard tidak mengubah core bot." /><pre className="code-block">DASHBOARD_ENABLED=false</pre><p className="muted-copy">Ubah melalui environment hosting, lalu restart service. Tidak ada data yang direset.</p></Card></div>;
}
...
export function BannerManagerPage() {
  ...
  const [uploaded, setUploaded] = useState<{ path: string; fileName: string; size: number; mime: string } | null>(null);
  useEffect(() => setValues(initial), [initial]);
  ...
  const upload = async (file: File) => {
    if (!/^image\/(png|jpeg|webp|gif)$/.test(file.type)) { notify("Gunakan PNG, JPG, WebP, atau GIF.", "error"); return; }
    if (!file.size || file.size > 8 * 1024 * 1024) { notify("Ukuran asset maksimal 8 MB.", "error"); return; }
    setUploading(true);
    try {
      const dataUrl = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result || "")); reader.onerror = () => reject(new Error("File tidak dapat dibaca.")); reader.readAsDataURL(file); });
      const result = await api.uploadDashboardAsset(file.name, dataUrl);
      setUploaded(result);
      notify("Asset berhasil diunggah. Asset lama tidak ditimpa.");
    } catch (error) { notify(error instanceof Error ? error.message : String(error), "error"); }
    finally { setUploading(false); }
  };
  const copyPath = async () => { if (!uploaded) return; await navigator.clipboard.writeText(uploaded.path); notify("Path asset disalin.", "info"); };
  const uploadedUrl = uploaded ? `/api/dashboard/assets/file?path=${encodeURIComponent(uploaded.path)}` : "";
  return <div className="page-stack page-enter"><PageHeader icon={FileImage} kicker="Content" title="Asset Manager" description="Upload, preview, dan kelola sumber gambar tanpa menimpa file lama." /><Card><CardHeader title="Upload asset" description="PNG, JPG, WebP, atau GIF • maksimal 8 MB • nama file dibuat unik." /><div className="asset-manager-grid"><label className="asset-upload-zone"><Upload size={22}/><strong>{uploading?"Uploading...":"Pilih image"}</strong><span>File disimpan sebagai asset baru.</span><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" disabled={uploading} onChange={(event)=>event.target.files?.[0]&&upload(event.target.files[0])}/></label>{uploaded?<div className="asset-preview-card"><img src={uploadedUrl} alt={uploaded.fileName}/><div><strong>{uploaded.fileName}</strong><span>{uploaded.mime} • {Math.ceil(uploaded.size/1024)} KB</span><code>{uploaded.path}</code><div className="preview-actions"><Button variant="secondary" icon={<Copy size={14}/>} onClick={copyPath}>Copy path</Button><Button variant="secondary" icon={<ExternalLink size={14}/>} onClick={()=>window.open(uploadedUrl,"_blank","noopener,noreferrer")}>Open</Button></div></div></div>:<div className="asset-empty-state"><FileImage size={24}/><strong>No upload selected</strong><span>Preview asset baru akan muncul di sini.</span></div>}</div></Card><Card><CardHeader title="Image sources" description="Untuk image Discord, gunakan URL HTTPS yang dapat diakses publik." /><div className="form-grid"><div className="form-field"><label>Banner MOTM</label><input value={values.motm} onChange={(event) => setValues({ ...values, motm: event.target.value })} placeholder="https://..." />{values.motm?<img className="field-image-preview" src={values.motm} alt="Preview banner MOTM"/>:null}</div><div className="form-field"><label>Image leaderboard lifetime</label><input value={values.leaderboard} onChange={(event) => setValues({ ...values, leaderboard: event.target.value })} placeholder="https://..." />{values.leaderboard?<img className="field-image-preview" src={values.leaderboard} alt="Preview leaderboard"/>:null}</div><div className="form-field"><label>Background dashboard custom</label><input value={values.dashboard} onChange={(event) => setValues({ ...values, dashboard: event.target.value })} placeholder="https://..." />{values.dashboard?<img className="field-image-preview" src={values.dashboard} alt="Preview background dashboard"/>:null}</div></div></Card><Card><CardHeader title="Usage" description="Lokasi yang memakai asset atau URL dari halaman ini." /><div className="backup-grid"><div><CheckCircle2 size={17}/><span>MOTM banner</span></div><div><CheckCircle2 size={17}/><span>Lifetime leaderboard</span></div><div><CheckCircle2 size={17}/><span>Dashboard background</span></div></div></Card>{dirty ? <div className="page-save-bar page-save-bar-dirty"><div><strong>Perubahan gambar belum disimpan</strong><span>Simpan URL baru atau batalkan untuk kembali.</span></div><div><Button variant="secondary" icon={<RefreshCcw size={16} />} onClick={cancel} disabled={saving}>Batal</Button><Button icon={<Save size={16} />} onClick={save} disabled={saving}>{saving ? "Menyimpan" : "Simpan"}</Button></div></div> : null}</div>;
}
...
export function KtpWargaPage() {
  ...
  return <div className="page-stack page-enter">
    ...
    <Card className="ktp-safety-card"><div className="info-panel"><ShieldCheck size={19}/><p><strong>KTP Warga hanya kartu identitas server.</strong> Jangan gunakan NIK asli, alamat lengkap, nomor telepon, tanggal lahir, atau data pribadi sensitif. Gunakan nama Discord, role, level, poin, hobi, dan lokasi umum saja.</p></div></Card>
    ...
    <div className="ktp-studio-grid">
      ...
      <div className="ktp-preview-column"><Card><CardHeader title="Preview langsung" description="Preview berskala dari layout renderer 1011 × 639. Simpan untuk menerapkan ke hasil Discord." />
        <div className="ktp-live-preview" style={{backgroundImage:`url('/api/dashboard/ktp/asset?path=${encodeURIComponent(values.backgroundPath)}')`}}>
          ...
          {values.design.fields.visible!==false?<div className="ktp-preview-fields" style={{left:`${values.design.fields.labelX/10.11}%`,top:`${(values.design.fields.startY-24)/6.39}%`,color:values.design.fields.valueColor,fontSize:`${values.design.fields.valueSize*.42}px`,lineHeight:`${values.design.fields.gap*.42}px`}}>{[["Display Name","Warga Tulus"],["Username","@warga_tulus"],["Role","Warga Desa"],["Level","Level 42"],["Points","12.450"],["Hobi / Lokasi","Mabar • Jawa Barat"]].map(([l,v])=><div key={l}><b>{l}</b><span>:</span><strong>{v}</strong></div>)}</div>:null}
          {values.design.photo.visible!==false?<div className="ktp-preview-photo" style={{left:`${values.design.photo.x/10.11}%`,top:`${values.design.photo.y/6.39}%`,width:`${values.design.photo.width/10.11}%`,height:`${values.design.photo.height/6.39}%`,borderRadius:values.design.photo.radius*.42,borderWidth:Math.max(1,values.design.photo.borderWidth*.42),borderColor:values.design.photo.borderColor,background:values.design.photo.frameColor}}>AVATAR</div>:null}
          {values.design.date.visible!==false?<div className="ktp-preview-date" style={previewStyle(values.design.date)}>Server Joined:<b>Juni 2026</b></div>:null}
