...
export function SettingsPage() {
  ...
  const [serverName, setServerName] = useState(data.config.serverName || "DESA TULUS");
  const [ownerName, setOwnerName] = useState(data.config.ownerName || "Pak RW");
  const [embedColor, setEmbedColor] = useState(data.config.embedColor || "#88a08c");
  const save = async () => { try { await api.savePatches([{ path: "serverName", value: serverName }, { path: "ownerName", value: ownerName }, { path: "embedColor", value: embedColor }]); await refresh(); notify("Settings berhasil disimpan."); } catch (error) { notify(error instanceof Error ? error.message : String(error), "error"); } };
  return <div className="page-stack page-enter"><PageHeader icon={Settings} kicker="System" title="Settings" description="Identitas dashboard dan nilai tampilan yang aman diubah." /><Card><CardHeader title="Identitas" description="Prefix publik tetap rw dan tidak diubah dari halaman ini." /><div className="form-grid two-columns"><div className="form-field"><label>Nama server</label><input value={serverName} onChange={(event) => setServerName(event.target.value)} /></div><div className="form-field"><label>Nama owner</label><input value={ownerName} onChange={(event) => setOwnerName(event.target.value)} /></div><div className="form-field"><label>Warna embed default</label><div className="color-field"><input type="color" value={embedColor} onChange={(event) => setEmbedColor(event.target.value)} /><input value={embedColor} onChange={(event) => setEmbedColor(event.target.value)} /></div></div><div className="form-field"><label>Prefix publik</label><input value={data.status.prefix} disabled /></div></div></Card><div className="page-save-bar"><div><strong>Simpan settings</strong><span>Core logic dan data aktif tidak diubah.</span></div><Button icon={<Save size={16} />} onClick={save}>Simpan perubahan</Button></div></div>;
}
